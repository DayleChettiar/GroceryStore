...
public class PhotoController extends BaseController {
    ...
    private static final String DEFAULT_PHOTO = "cartItem-photo.png";
    
    @Value("${cartItems.photo.storage}")
    private String photoDir;
    
    @Value("${cartItems.photo.resize-width}")
    private int photoWidth;
    
    @Value("${cartItems.photo.resize-height}")
    private int photoHeight;
    
    @Autowired
    private MultipartResolver multipartResolver;
    
    @RequestMapping(value="/{cartItemId}", method = GET)
    public HttpEntity<byte[]> getPhoto(@PathVariable String cartItemId, WebRequest request) throws IOException {
        File photoFile = new File(photoDir, cartItemId + "." + EXT_NAME);
        if (!photoFile.exists()) {
    ...
    @RequestMapping(value="/{cartItemId}", method = POST)
    public void uploadPhoto(@PathVariable String cartItemId, HttpServletRequest request) throws IOException {
        MultipartHttpServletRequest multipartRequest = multipartResolver.resolveMultipart(request);
        
        MultipartFile file = multipartRequest.getFile("file");
        File uploadFile = File.createTempFile("cartItem-", cartItemId);
